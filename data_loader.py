...
class ConditionAlignmentDataset(Dataset):
    """用于条件对齐的数据集加载器
    
    加载不同视觉条件下的图像，并将它们组织成对，用于训练条件对齐模块(CAM)。
    """
    def __init__(self, root_dir, target_condition, source_conditions, split='train', transform=None):
        ...
        Args:
            ...
            source_conditions (list): 源条件类型列表，如['canny', 'sketch']
    ...
    def __getitem__(self, idx):
        ...
        # 随机选择一个源条件
        source_condition = random.choice(self.source_conditions)
        source_path = os.path.join(
            self.root_dir, 
            f"img_{source_condition}", 
            self.split, 
            class_name, 
            img_name
        )
